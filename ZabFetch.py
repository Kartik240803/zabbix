...
class QueryBuilder:
    ...
    @staticmethod
    def build_item_detail_query(with_hostname: bool) -> str:
        base_query = """
        ...
        """
        if with_hostname:
            return base_query + "WHERE h.host = %s AND i.name = %s"
        return base_query + "WHERE i.name = %s AND h.status = 0 AND h.flags IN (0, 4)"
...
class ZabbixDB:
    ...
    def get_host_by_group(self, host_group: str) -> List[Dict[str, Any]]:
        ...
        try:
            with self.connection.cursor(dictionary=True) as cursor:
                cursor.execute(query, (host_group,) if host_group.lower() != 'all' else None)
                return cursor.fetchall() or []
        except (MySQLError, PostgresError) as e:
            logger.error(f"Query failed in get_host_by_group: {str(e)}")
            raise ZabbixDBError(f"Query failed: {str(e)}")

    def get_item_detail(self, item_name: str, hostname: str = None) -> Union[Dict[str, Any], List[Dict[str, Any]], None]:
        """Fetch item details."""
        self._ensure_connection()
        
        try:
            with self.connection.cursor(dictionary=True) as cursor:
                query = QueryBuilder.build_item_detail_query(bool(hostname))
                params = (hostname, item_name) if hostname else (item_name,)
                ...
                return map_item(results[0]) if hostname else [map_item(item) for item in results]
        except (MySQLError, PostgresError) as e:
